...
const SvarMotebehovInnhold = ({ svarMotebehov, motebehovReducer, motebehovSvarReducer }) => {
  const visKvittering = skalViseMotebehovKvittering(motebehovReducer);
  const title = visKvittering ? texts.title.receipt : texts.title.default;
  const content = visKvittering ? (
    <SvarMotebehovKvitteringSide motebehovReducer={motebehovReducer} />
  ) : (
    <MotebehovSvar motebehovSvarReducer={motebehovSvarReducer} svarMotebehov={svarMotebehov} />
  );
  return (
    <React.Fragment>
