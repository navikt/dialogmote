...
const getPerioder = (type) => {
    ...
    const setPeriodeDate = (periode, type) => {
        periode.fom = leggTilDagerPaaDato(today, -(type.fomUke * 7)).toJSON();
        periode.tom = leggTilDagerPaaDato(today, ((type.tomUke - type.fomUke) * 7)).toJSON();
        return periode;
    };
    return [setPeriodeDate(perioder[0], type)];
};
